...
type PriceFilterProps = {
  open: boolean;
  handleClose: () => void;
  value?: [number, number]; // current selection from parent
  onChange: (val: [number, number]) => void; // callback to parent
};

const PriceFilter: React.FC<PriceFilterProps> = ({
  open,
  handleClose,
  value: parentValue = [50, 3000],
  onChange,
}) => {
  ...
  const handleSave = () => {
    onChange(value); // send value back to parent
    handleClose();
  };
  ...
  return (
    <Modal open={open} onClose={handleClose}>
      <Box sx={style}>
        ...
        {/* Labels Min/Max */}
        <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 4, marginBottom: 1 }}>
